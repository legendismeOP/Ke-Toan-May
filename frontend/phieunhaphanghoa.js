...
$(document).ready(function () {
    ...
    function load_data() {
        $.ajax({
            method: "GET",
            dataType: "json",
            url: "http://localhost:3000/phieunhaphanghoa",
            success: function (data){
                    ...
                    data.forEach(item => {
                        html += `
                            <tr>
                                ...
                                <td>`+item.cMaNguoiBan+`</td>
                                <td>`+item.cTenNguoiBan+`</td>
                                <td>`+item.cMaSoThueNguoiBan+`$</td>
                                <td>`+item.cTaiKhoanNo+`</td>
                                <td>`+item.cTaiKhoanNoGTGT+`</td>
                                <td>`+item.cTaiKhoanCo+`</td>
                ...
                $('#phieunhaphanghoa tbody').html(html);
                //console.log(html)
            }
        });
    };
    ...
    $(document).on('click', '.delete', function(){
        ...
        {
            $.ajax({
                url:"http://localhost:3000/phieunhaphanghoa/deletephieunhaphanghoa/"+cMaChungTu,
                method:"DELETE",
                success:function()
                {
